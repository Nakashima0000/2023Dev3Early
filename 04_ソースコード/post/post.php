<?php
    ...
    require '../php/DB.php';
    $up_post = new up_post;
    $uplord_img = new uplord_img;
    
    if(isset($_REQUEST["POST_TOKEN"])){
        if ($_REQUEST["POST_TOKEN"] === $_SESSION["POST_TOKEN"]) {
            if(isset($_POST['btn'])){
                $intro = $_POST['intro'];
                $uplord_post = $up_post->uplord_post($me,$intro);
                if (isset($_FILES['img'])) {
                    //$ftpimg = $ftp_img->ftpimg($_FILES);
                    for($i = 0; $i < count($_FILES["img"]["name"]); $i++ ){
                        if($_FILES['img']['tmp_name'][$i] != null){
                            $image = uniqid(mt_rand(), true);//ファイル名をユニーク化
                            $image .= '.' . substr(strrchr($_FILES['img']['name'][$i], '.'), 1);//アップロードされたファイルの拡張子を取得
                            $file = "http://nakash.main.jp/system5/img/$image";
                            if (!empty($_FILES['img']['name'][$i])) {//ファイルが選択されていれば$imageにファイル名を代入
                                move_uploaded_file($_FILES['img']['tmp_name'][$i], '../img/' . $image);//imagesディレクトリにファイル保存
                            }
                            $_FILES['img']['tmp_name'][$i] = null;
                            $uplord_img->upimg($uplord_post,$file);
                        }else{
    ...
    <body>
        ...
        <div class="main">
            <form method="post" enctype="multipart/form-data">
                <div class="post_btn">
                    <button type="submit" name="btn" class="post_button">投稿</button>
                </div>
                ...
                <div class="post_sel">
                    <label class="sel">
                        <p>＋</p>
                        <input type="file" name="img[]" id="form" class="up_sel" onchange="selectimg(this)" accept=".jpg, .jpeg, .png" multiple>
                    </label>
                </div>
                <div class="text_area">
                    <textarea name="intro" rows="10" maxlength="300" placeholder="キャプションを入力..." required></textarea>
                </div>
        ...
        <div class="container-fluid footer">
            <a href="../home/home.php" style="all:initial;">
